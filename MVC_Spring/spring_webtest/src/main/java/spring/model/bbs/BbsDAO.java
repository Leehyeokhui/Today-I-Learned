...
//	}
	...
	/*
	 * public List<BbsDTO> list(Map map) { List<BbsDTO> list = new
	 * ArrayList<BbsDTO>(); Connection con = DBOpen.open(); PreparedStatement pstmt
	 * = null; ResultSet rs = null;
	 * 
	 * String col = (String) map.get("col"); String word = (String) map.get("word");
	 * int sno = (Integer) map.get("sno"); int eno = (Integer) map.get("eno");
	 * StringBuffer sql = new StringBuffer();
	 ...
	 * sql.append("       FROM bbs   "); if (word.trim().length() > 0 &&
	 * col.equals("title_content")) { sql.append(" where title like '%'||?||'%' ");
	 * sql.append(" or content like '%'||?||'%' "); } else if (word.trim().length()
	 * > 0) { sql.append(" where " + col + " like '%'||?||'%' "); }
	 * 
	 * sql.append("     order by grpno desc, ansnum ASC "); sql.append("  )  ");
	 * sql.append(" )WHERE r>=? and r<=?  "); try { pstmt =
	 * con.prepareStatement(sql.toString()); int i = 0; if (word.trim().length() > 0
	 * && col.equals("title_content")) { pstmt.setString(++i, word);
	 * pstmt.setString(++i, word); } else if (word.trim().length() > 0) {
	 * pstmt.setString(++i, word); } pstmt.setInt(++i, sno); pstmt.setInt(++i, eno);
	 * rs = pstmt.executeQuery();
	 * 
	 * while (rs.next()) { BbsDTO dto = new BbsDTO();
	 * dto.setBbsno(rs.getInt("bbsno")); dto.setWname(rs.getString("wname"));
	 * dto.setTitle(rs.getString("title")); dto.setWdate(rs.getString("wdate"));
	 * dto.setGrpno(rs.getInt("grpno")); dto.setIndent(rs.getInt("indent"));
	 * dto.setAnsnum(rs.getInt("ansnum"));
	 * dto.setFilename(rs.getString("filename"));
	 * 
	 * list.add(dto); }
	 ...
	 * e.printStackTrace(); } finally { DBClose.close(con, pstmt, rs); }
	 * 
	 * return list; }
	 */
